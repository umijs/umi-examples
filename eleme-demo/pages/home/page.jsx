...
export default class extends React.Component {
  ...
  loadTypeData({ latitude, longitude }) {
    fetch(`/shopping/openapi/entries?latitude=${latitude}&longitude=${longitude}&templates[]=main_template&templates[]=favourable_template&templates[]=svip_template`)
      .then(res => {
        if (res.status === 200) {
          res.json().then(data => {
            this.setState({
  ...
  */
  loadRestaurantData({ latitude, longitude }, rank_id = '') {
    fetch(`/shopping/v3/restaurants?latitude=${latitude}&longitude=${longitude}&offset=${this.rests.length}limit=8&extras[]=activities&extras[]=tags&extra_filters=home&rank_id=&terminal=h5&rank_id=${rank_id}`)
      .then(res => {
        if (res.status === 200) {
          res.json().then(data => {
      ...
      <div className={styles.home}>
        ...
        <StickyContainer>
          ...
          <section className={styles.toptoon}>
            <img src="https://fuss10.elemecdn.com/0/cf/e16c1687a4ea84674d5b531623934png.png?imageMogr/format/webp/thumbnail/!750x210r/gravity/Center/crop/750x210/" alt="toptoon" />
          </section>
          <Grid
